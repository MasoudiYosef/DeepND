...
# Residual block consisting of two convolutional layers and a shortcut connection
def basic_block(x, filters, stride=1):
    ...
    # Match dimensions when filter number or stride changes
...
# Build the DeepND residual network architecture
...
# Load positive and negative datasets and generate training/testing sets
def Prepare_Data(FN1,FN2,INX):
    ...
    # Generate training set from positive samples
    ...
    # Generate test set from positive samples
    ...
    # Generate training set from negative samples
    ...
    # Generate test set from negative samples
    for i in range(0,len(SL)):
        k=DT2[SL[i]].split(',')
        ls=[]
    ...
    # Reshape inputs for neural network training
...
for INX in range(ST,ST+1,10):
    for D in ND:
        ...
        for it in range(1,31):
            ...
            # Candidate optimizers used during recovery from overfitting
            OPT=['sgd','rmsprop','adam','adadelta',
            ...
            for epoch in range(ne):
                ...
                # Save model snapshot when validation accuracy improves
                if (res[1]<re[1]):

                    p=0

                    print('Improvement',epoch,re[1])
                ...
                # Restore best snapshot and switch optimizer if no improvement
                elif p>3:
